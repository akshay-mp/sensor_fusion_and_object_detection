...
def show_pcl(pcl):
    ...
    vis_lpc.register_key_callback(262,closeWindow)
...
def show_range_image(frame, lidar_name):
    ...
    ri_intensity = ri[:,:,1]
    p1,p99=percentile(ri_intensity,1),percentile(ri_intensity,99)
    ri_intensity =  255 * ((np.clip(ri_intensity,p1,p99))/(p99))
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_pcl_cpy[:, 1] = abs(np.int_(np.floor(lidar_pcl_cpy[:, 1] / bev_discret)) +((configs.bev_width + 1) / 2))
